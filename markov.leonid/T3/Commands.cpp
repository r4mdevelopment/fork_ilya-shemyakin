...
namespace mrkv
{
  ...
  int count(std::vector<Polygon>& figures, std::string& arg)
  {
	...
	if (num == -1)
	{
	  if (arg == "EVEN")
	  {
		return std::count_if(figures.begin(), figures.end(), std::bind(count—ondition, _1, 2, 0));
	  }
	  else if (arg == "ODD")
	  {
		return std::count_if(figures.begin(), figures.end(), std::bind(count—ondition, _1, 2, 1));
	  }
	  else
	  {
		throw std::invalid_argument("<INVALID COMMAND>");
	...
	{
	  return std::count_if(figures.begin(), figures.end(), std::bind(count—ondition, _1, num, 0));
	}
	else
	{
	  throw std::invalid_argument("<INVALID COMMAND>");
